...
const TAB_HINT: Partial<Record<BlockType, string>> = {
  action:   'Character',
  character: 'Action',
  dialogue:  'new Character',
}

const ENTER_HINT: Partial<Record<BlockType, string>> = {
...
const ALL_TYPES: Array<{ type: BlockType; label: string }> = [
...
export default function ScriptEditor({ blocks, onChange, readOnly = false, knownCharacters = [], onSave }: Props) {
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLTextAreaElement>, block: ScriptElement) => {
      ...
      // ── Global shortcuts ──────────────────────────────────────────
      // Cmd+Shift+L → new scene heading  (L = sLug line)
      // Note: Cmd+Shift+H is intercepted by browsers as "Go to Homepage"
  ...
  return (
    <>
      ...
      {/* Sticky element type status bar */}
      {focusedBlock && (
        <ElementStatusBar
          block={focusedBlock}
          onChangeType={(newType) => {
            ...
            pushHistory(blocks)
            const text =
              newType === 'scene_heading' || newType === 'character' || newType === 'transition'
                ? focusedBlock.text.toUpperCase()
                : focusedBlock.text
            update(focusedBlock.id, { type: newType, text })
          }}
        />
      )}
...
function ElementStatusBar({
  ...
  const tabHint  = TAB_HINT[block.type]
  const enterHint = ENTER_HINT[block.type]

  return (
    // Breaks out of the parent's px-8 py-8 lg:pl-32 padding so it spans full width
    <div className="sticky bottom-0 -mx-8 -mb-8 lg:-ml-32 z-20 bg-zinc-950/95 backdrop-blur-sm border-t border-zinc-800 flex items-center gap-6 px-8 lg:pl-32 py-2 text-xs select-none">
      {/* Key hints */}
      <div className="flex items-center gap-5 text-zinc-600">
        {tabHint && (
          <span className="flex items-center gap-1.5">
            <kbd className="font-mono bg-zinc-800 text-zinc-400 px-1.5 py-0.5 rounded border border-zinc-700 text-[10px]">Tab</kbd>
        ...
        {enterHint && (
          <span className="flex items-center gap-1.5">
            <kbd className="font-mono bg-zinc-800 text-zinc-400 px-1.5 py-0.5 rounded border border-zinc-700 text-[10px]">Enter</kbd>
            <span className="text-zinc-700">→</span>
            <span>{enterHint}</span>
          </span>
        )}
      ...
      <div className="relative">
        ...
        {open && (
          <>
            ...
            <div className="absolute bottom-full right-0 mb-2 z-40 bg-zinc-900 border border-zinc-700 rounded-lg shadow-2xl overflow-hidden min-w-[160px]">
...
const QUICK_KEYS = [
  ...
  },
  {
    section: 'Insert',
    keys: [
      { keys: '⌘ Shift L', where: 'Anywhere', result: 'Insert scene heading after current block' },
      { keys: '⌘ Shift T', where: 'Anywhere', result: 'Insert transition after current block' },
      { keys: 'Element bar →', where: 'Focused block', result: 'Click type label to change element type' },
    ],
  },
  {
  ...
  return (
    <div className={`relative group w-full ${compact ? 'pt-0 pb-0.5' : 'py-0.5 mt-0.5'}`}>
      ...
      <textarea
        ...
        onKeyDown={onKeyDown}
        onFocus={() => { setFocused(true); onFocusBlock?.(block.id) }}
